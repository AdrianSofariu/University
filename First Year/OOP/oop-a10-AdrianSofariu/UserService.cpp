...
std::vector<Movie> UserService::getWatchlistMovies()
{
	return this->watchlist.getRepo();
}
...
void UserService::removeFromWatchlist(const std::string& title)
{
	Movie aux(title);
	this->watchlist.deleteMovie(aux);
}

void UserService::likeMovie(const std::string& title)
{
	Movie movie(title);
	Movie* found = this->repo.findMovie(movie);
	if (found == NULL)
		throw InexistentMovieException();
	movie.setGenre(found->getGenre());
	movie.setLikes(found->getLikes() + 1);
	movie.setYear(found->getYear());
	movie.setTrailer(found->getTrailer());
	//found->setLikes(found->getLikes() + 1);
	this->repo.updateMovie(*found, movie);
}
...
void testUserService()
{
	...
	userserv.removeFromWatchlist(movie1.getTitle());
	...
	// Test likeMovie
	userserv.likeMovie(m1.getTitle());
