...
class Grade(IdObject):
    ...
    def __str__(self):
        if self.grade_value != -1:
            return ("  Student: " + str(self.student_id) + "  Assignment: " + str(self.__assignment_id)
    ...
    @staticmethod
    def generate_grades(n: int) -> tuple:
        """
        ...
        while i < n:
            ...
            a_id = random.choice(assignments).id
            ...
            if ok == 1:
                result.append(g)
                i += 1
        return students, assignments, result
